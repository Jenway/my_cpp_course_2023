...
public:
    ...
    void setRegularGrade(float regularGrade) { this->regularGrade = regularGrade; }
    void setFinalExam(float finalExam) { this->finalExam = finalExam; }
...
void Modify(Student stu[], int &currentN)
{
    int id;
    cout << "请输入需要修改的学生学号:";
    cin >> id;
    for (int i = 0; i < currentN; i++)
    {
        if (stu[i].getId() == id)
        {
            ...
            float dailyMark;
            cin >> dailyMark;
            stu[i].setRegularGrade(dailyMark);
            cout << "请输入新的期末成绩:";
            float finalExam;
            cin >> finalExam;
            stu[i].setFinalExam(finalExam);
            cout << "修改成功!" << endl;
            return;
        }
    }
